...
➖➖➖➖➖➖➖➖
    """
        ...
        for i in range(2, len(__service)-1):
            ...
            msg += f'\n{await self.get_country_name(__service[i].split(":")[0])} | {qty[f"{code}_0"]} | {__service[i].split(":")[1]} ₽'
    ...
    async def get_menu(self, code):
        ...
        for i in range(2, len(__service)):
            if __service[i] == '0':
                break
            menu.add(
                types.InlineKeyboardButton(text=f'{await self.get_country_name(__service[i].split(":")[0])} | {__service[i].split(":")[1]} ₽',
